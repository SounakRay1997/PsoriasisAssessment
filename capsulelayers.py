...
class CapsuleLayer(layers.Layer):
    ...
    def call(self, inputs, training=None):
        ...
        #b = tf.ones(shape=[K.shape(inputs_hat)[0], self.num_capsule, self.input_num_capsule])
...
def PrimaryCap(inputs, dim_capsule, n_channels, kernel_size, strides, padding,region=''):
    """
    ...
    output = layers.Conv2D(filters=dim_capsule*n_channels, kernel_size=kernel_size, strides=strides, padding=padding,
                           name='primarycap_conv2d'+str(region))(inputs)
                           
    outputs = layers.Reshape(target_shape=[-1, dim_capsule], name='primarycap_reshape'+str(region))(output)
